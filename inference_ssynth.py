...
def predict_cond_model_sec_approach_list_full_wav(Textgrid_path, Energy_npy_path, cur_model, device, sampling_rate=16000):
    ...
    while not(finished):
        ...
        if(initial_frame):
            ...
            generated_final_audio, sr = inference_process(conditioned_audio, conditioned_phonemes_signal, conditioned_energy_signal, cur_model, device = device)
            initial_frame = False
        else:
            total_length_curr_window = int(round(end_cur_phonemes-start_cur_phonemes))
            conditioned_audio = torch.zeros((1,total_length_curr_window))
            ...
            generated_current_audio, sr = inference_process(conditioned_audio, conditioned_phonemes_signal, conditioned_energy_signal, cur_model, device = device)
            debug_data_to_save[i] = [conditioned_audio.detach().cpu(), generated_current_audio.detach().cpu(), overlap_duration]
    ...
    debug_nm = Energy_npy_path.split('Energy_predicted_')[-1][:-4]
    import pickle
    pickle.dump(debug_data_to_save, open(f'tmp_debug_data_{debug_nm}.pickle','wb'))
...
### predict several wavs from a folder: 
    ...
    if True:
        ...
        params.residual_layers = 32
        current_model =  os.path.join(os.getcwd(),"runs_ssynth/DiffAR_200/outputs/exp_1/weights.pt")
        dev = torch.device('cuda',2)
        checkpoint = torch.load(current_model)
        params.noise_schedule = np.linspace(**params.noise_schedule,).tolist()
        model = DiffAR(params).to(dev) # TODO!!!! I loaded it in ipython..
        model.load_state_dict(checkpoint['model'])
        cur_model_ = model
    ...
    for cur_iter in range(iter_per_file):
        ...
        for files in os.listdir(Textgrid_path):
            ...
            if files.endswith(ext):
                ...
                t0= time.time()
                original_audio, generated_final_audio, sr = predict_cond_model_sec_approach_list_full_wav(cur_TextGrid_path, cur_Energy_npy_path, cur_model_, dev)
                t1 = time.time() - t0
                torchaudio.save(f"{target_path}/generated_{cur_iter}_{file_name}.wav", generated_final_audio.cpu(), sample_rate=16000)
                #--- this is just the initial conditioning audio (a second of silence)
                #torchaudio.save(f"{target_path}/orig_{cur_iter}_{file_name}.wav", original_audio.cpu(), sample_rate=16000)
